...
3. Before implementing nontrivial work the human has not approved: produce a short plan (chat or `plan.md`). Prefer interactive review over a wall of markdown. Offer Plannotator once for long plans or spike docs; do not auto-launch. Wait for implement approval.
4. On approval: plan file is read-only; finish every todo. Parent orchestrates; Composer minions implement nontrivial code per the Planner and workers delta.
5. Step 5 verify is legacy-aware per Cameron Verification delta (tests when they exist; live/manual repro when they do not). Heavy verify may fan to a minion; parent owns done.
