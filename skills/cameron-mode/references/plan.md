...
Produce a phased implementation plan grounded in the **Principles** section of the `cameron-mode` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `cameron-mode` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.

## 2. Scope and constraints

State your read of scope and constraints in one paragraph. Use `AskQuestion` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); ask one question at a time with concrete options.
...
- Prefer `subagent_type: "cameron-agent"`. `generalPurpose` is the fallback. Never use the built-in `plan` subagent_type; it ignores this skill.
- Pass `model:` explicitly per the configured roles in `pstack-models.mdc`.
...
In the overview, name which cameron-mode non-negotiables the implementer must apply, by name:
...
- **Opening an ADO PR** after shipping each phase when code ships.
